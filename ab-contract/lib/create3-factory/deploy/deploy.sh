...
deploy() {
	NETWORK=$1
	RAW_RETURN_DATA=$(forge script script/Deploy.s.sol -f $NETWORK -vvvvv --json  --silent --broadcast --skip-simulation --legacy)
	...
	echo "CREATE3Factory deployed on address： "  $factory
