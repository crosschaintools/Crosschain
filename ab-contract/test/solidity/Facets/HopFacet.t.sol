...
import { IApp, LibSwap, LibAllowList, TestBaseFacet, console, ERC20 } from "../utils/TestBaseFacet.sol";
import { HopFacet } from "app/Facets/HopFacet.sol";
...
contract HopFacetTest is TestBaseFacet {
    ...
    event HopInitialized(HopFacet.HopConfig[] configs);
    ...
    IApp.BridgeData internal validBridgeData;
    ...
    function setUp() public {
        ...
        HopFacet.HopConfig[] memory configs = new HopFacet.HopConfig[](3);
        configs[0] = HopFacet.HopConfig(ADDRESS_USDC, USDC_BRIDGE);
        configs[1] = HopFacet.HopConfig(ADDRESS_DAI, DAI_BRIDGE);
        configs[2] = HopFacet.HopConfig(address(0), NATIVE_BRIDGE);
    ...
    function initiateBridgeTxWithFacet(bool isNative) internal override {
        if (isNative) {
            hopFacet.startBridgeTokensViaHop{ value: bridgeData.preBridgeAmount }(
                bridgeData,
                validHopData
            );
        } else {
    ...
    function testRevert_ReentrantCallBridge() public {
        ...
        bridgeData.preBridgeAmount = 1 ether;
    ...
    function testRevert_ReentrantCallBridgeAndSwap() public {
        ...
        bridgeData.preBridgeAmount = amountOut;
    ...
    function testRevert_NotEnoughMsgValue() public {
        ...
        hopFacet.startBridgeTokensViaHop{ value: bridgeData.preBridgeAmount - 1 }(
            bridgeData,
            validHopData
        );
    ...
    function testRevert_RegisterBridgeWithUninitializedFacet() public {
        vm.startPrank(USER_DIAMOND_OWNER);
        Diamond diamond2 = createDiamond();
    ...
    function test_BridgeFromL2ToL1() public {
        ...
        HopFacet.HopConfig[] memory configs = new HopFacet.HopConfig[](1);
        configs[0] = HopFacet.HopConfig(
        ...
        vm.expectEmit(true, true, true, true, address(hopFacet2));
        emit TransferStarted(bridgeData);
