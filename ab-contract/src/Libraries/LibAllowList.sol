...
library LibAllowList {
    ...
    bytes32 internal constant NAMESPACE =
        keccak256("com.aggbridge.library.allow.list");
    ...
    function _checkAddress(address _contract) private view {
        if (_contract == address(0)) revert InvalidContract(_contract);

        if (_contract.code.length == 0) revert InvalidContract(_contract);
    }
}
