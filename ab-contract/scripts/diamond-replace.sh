...
update() {
  ...
  # check if env variable "PRODUCTION" is true, otherwise deploy as staging
	...
	echo "Please select which diamond to replace:"
  ...
  if [[ "$SELECTION" == *"default"* ]]; then
	...
	SCRIPT=$(ls -1 script | sed -e 's/\.s.sol$//' | grep 'Replace' | gum filter --placeholder "Diamond Replace Script")
...
checkFailure() {
	if [[ $? -ne 0 ]]; then
		echo "Failed to replace diamond"
		exit 1
	fi
}
