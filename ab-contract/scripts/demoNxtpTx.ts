...
async function main() {
  ...
  const diamond = NXTPFacet__factory.connect(Diamond_ADDRESS, wallet)
  ...
  if (WITH_RECEIVER) {
    ...
    const tmpTx = await diamond.populateTransaction.completeBridgeTokensViaNXTP(
      diamondData,
  ...
  if (WITH_SWAP) {
    ...
    await diamond.swapAndStartBridgeTokensViaNXTP(
      diamondData,
      [
        {
    ...
    // Call Diamond smart contract to start the bridge process -- WITHOUT SWAP
    await diamond.startBridgeTokensViaNXTP(diamondData, nxtpData, {
      gasLimit: 500000,
    })
  }
