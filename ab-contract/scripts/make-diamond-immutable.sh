...
update() {
  ...
  # get diamond address from path (finds any key that contains "DiamondImmutable", works with versioning (V1, V2 etc.)
  DIAMOND=$(jq 'to_entries[] | select(.key | contains("DiamondImmutable")) | .value' $ADDRS)
