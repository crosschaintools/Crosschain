...
import { SquidFacet__factory, IApp, SquidFacet, ERC20__factory, ISquidRouter__factory } from '../typechain'
...
const main = async () => {
  ...
  const squidFacet = SquidFacet__factory.connect(Diamond_ADDRESS, provider)
  ...
  let tx = await token.connect(signer).approve(Diamond_ADDRESS, bridgeData.minAmount, { maxFeePerGas, maxPriorityFeePerGas })
  await tx.wait()
